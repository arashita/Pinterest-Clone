# **USER PERSONAS**

## User Persona: Jyo Cho (Influencer)

- Name: Jyo Cho
- Occupation: Home Decor & Lifestyle Influencer
- Location: Los Angeles, USA
- Background:Jyo Cho is a well-known home decor and lifestyle influencer who shares high-quality, aesthetically pleasing images of home interiors, travel inspirations, and lifestyle tips. She collaborates with brands and engages with a large audience through Pinterest.

### Goals & Motivations:

- Inspire followers by sharing visually engaging content.
- Increase audience engagement through likes and comments.
- Organize content efficiently with multiple boards and categories.
- Create a professional, visually cohesive profile that aligns with her brand.

### Pain Points & Challenges:

- Managing a high volume of likes and comments.
- Keeping posts and boards well-organized for easy navigation.
- Dealing with spam or negative comments.
- Ensuring consistent engagement while balancing brand collaborations.

### Platform Usage Behavior:

📌 Daily Activity:

1. Uploads curated images with detailed captions.
2. Engages with followers by responding to comments.
3. Organizes posts into themed boards (e.g., "Minimalist Decor," "Travel Aesthetics").

📌 Interaction Style:

- Prefers professional and engaging interactions.
- Deletes or moderates comments that are off-brand.
- Actively seeks high engagement on posts to attract collaborations.

### Features Jyo Cho Needs in the Pinterest:

- User Authentication – Secure login/logout.
- Board Creation & Management – Organize posts into structured collections.
- High-Quality Image Uploads – Showcase content in the best resolution.
- Likes & Engagement Tracking – See which posts perform best.
- Comment Moderation – Delete or reply to comments (threaded discussions).

## User Persona: Arashita (Creative & Developer)

- Name: Arashita
- Occupation: Software Developer & Creative Enthusiast
- Location: Bangalore, India
- Background:Arashita is a software developer with a passion for creativity, design, and personal projects. She uses Pinterest-like platforms to explore visual inspirations, organize creative ideas, and share her own work. She enjoys discovering UI/UX designs, coding-related visual content, and aesthetic inspirations for projects.

### Goals & Motivations:

- Store and organize inspirations for personal and technical projects.
- Upload and share creative work such as UI designs, project screenshots, and artwork.
- Engage with like-minded creators by liking and commenting on posts.
- Use boards to categorize ideas for better organization.
- Discuss technical and design concepts through threaded comments.

### Pain Points & Challenges:

- Finding niche content that aligns with specific technical and design interests.
- Managing different creative inspirations across multiple boards.
- Keeping track of discussions in threaded comments.
- Avoiding clutter and maintaining a clean, organized profile.

### Platform Usage Behavior:

📌 Daily Activity:

- Uploads UI/UX design ideas, coding visuals, and project inspirations.
- Organizes posts into structured boards (e.g., "Web Design," "Game UI," "Python Projects").
- Likes and comments on posts from other developers and designers.

📌 Interaction Style:

- Engages in meaningful discussions through comments and replies.
- Looks for organized and structured content for quick access.
- Uses the platform for both inspiration and knowledge-sharing.

### Features Arashita Needs in the Pinterest:

- User Authentication – Secure login/logout.
- Board Creation & Management – Organize creative inspirations efficiently.
- Image Uploading – Share personal projects, designs, and coding-related visuals.
- Likes & Comments – Interact with other creators.
- Threaded Comments – Discuss design and coding ideas in-depth.
